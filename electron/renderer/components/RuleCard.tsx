    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {fixText && (
            <div
              className="mt-3 pr-3 py-2 border-r-2"
          ...
          {hasExtraDetails && (
            <div className="mt-2">
              ...
              >
                {showDetails ? 'مخفی کردن جزئیات' : `مشاهده جزئیات (${listFields.reduce((n, f) => n + f.items.length, 0) + statFields.length} مورد)`}
              </button>
                      ...
                      {statFields.map(({ key, value }) => {
                        ...
                        return (
                          <span key={key} className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                            ...
                            {typeof value === 'boolean' ? (value ? 'بله' : 'خیر') : String(value)}
                          </span>
                        );
                      })}
                    </div>
