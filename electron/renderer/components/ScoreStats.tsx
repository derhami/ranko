...
export function ScoreStats({ passCount, warnCount, failCount }: ScoreStatsProps) {
  return (
    <div className="flex gap-4">
      <StatBadge count={failCount} label="خطای بحرانی" colorVar="--color-fail" bgVar="--color-fail-bg" />
      <StatBadge count={warnCount} label="هشدار" colorVar="--color-warn" bgVar="--color-warn-bg" />
      <StatBadge count={passCount} label="پاس شده" colorVar="--color-pass" bgVar="--color-pass-bg" />
    </div>
  );
}
